...
// 		get_input_data_t1();
	...
	if( mode == "fit"){
		read_options();
		get_plots();
		if(start <= 0 && stop > 0) fit_table_k0(0); // pi-
		if(start <= 1 && stop > 1) fit_table_k0(1); // pi+
		if(start <= 2 && stop > 2) fit_table_phi(0); // k-
		if(start <= 3 && stop > 3) fit_table_phi(1); // k+
		if(start <= 4 && stop > 4) fit_table_lambda(0); // p-
		if(start <= 5 && stop > 5) fit_table_lambda(1); // p+
		if(start <= 6 && stop > 6) fit_table_rho(0); // pi-
		if(start <= 7 && stop > 7) fit_table_rho(1); // pi+
	 	print_table();
...
// 	opt["LH_athr_p_p:"]  = make_pair(3,2);
	...
	while(true){
		...
		if( line[0] != '#' && !line.empty() ) {
			...
			if (var1 == "hist_file:")		hist_file = var2;
	...
	for(int i = 6; i<8; i++){      //8
		...
		for(int p = 0; p<Np;p++){
			for(int t = 0; t<Nt; t++){
				...
				h[i][j][p][t] = new TH1D(nn.str().c_str(),"",Nbins[i],800,1050);
				h[i][j][p][t]->Sumw2();
			}
		}
	}
...
void write_hist(){
	...
	TFile* output = new TFile(hist_file.c_str(),"RECREATE");
...
// 		for(int p = 0; p<2; p++){
 		//Signal
			...
			RooRelBreitWigner sb("sb","relBW",x,mean,width,1.,1.,0.4937,0.4937);
...
// 		for(int t = 1; t<2; t++){
			...
			RooRealVar x2("x2","M",800.,1050,"MeV");
			x2.setBins(100);
			
			RooDataHist data_ks("data_ks","data_ks",x2,Import(*h[6+cc][5][p][t]));
			...
			// RooFormulaVar k_sigma2("k_sigma2","k_sigma1*k_frac_s",RooArgSet(k_sigma1,k_frac_s));
			
			RooGaussian k_gauss1("k_gauss1","gauss1",x2,k_mean,k_sigma1) ;
			RooGaussian k_gauss2("k_gauss2","gauss2",x2,k_mean,k_sigma2) ;
			
			RooAddPdf k_sig("k_sig","sig",RooArgList(k_gauss1,k_gauss2),k_frac_s) ;
			// RooRealVar k_n("k_n","n",  3.,  0.,   5.) ;
			// RooRealVar k_a("k_a","a",  0.01,   0.,    .1) ;
			// RooRealVar kpi_thr("kpi_thr","threshold",633.25);
			// RooGenericPdf k_bgn("k_bgn","x2<kpi_thr ? 0 :TMath::Power(x2 - kpi_thr,k_n)*TMath::Exp(-k_a*(x2 - kpi_thr))",RooArgList(x2,kpi_thr,k_n,k_a));
...
// 			RooRealVar k3pi("k3pi","k3pi",	0.2,	-1.,	1.) ;
// 			RooRealVar k4pi("k4pi","k4pi",	0.07,	-1.,	1.) ;
			RooChebychev k_bgn("k_bgn","k_bgn",x2,RooArgSet(k_k0,k_k1));
			
			//Construct composite pdf
			Int_t k_ent = h[6+cc][5][p][t]->GetEntries(); //[p][t]
			RooRealVar k_N_s("k_N_s","N_a_s",0.2*k_ent,0.,1.05*k_ent) ;
			RooRealVar k_N_b("k_N_b","N_a_b",0.8*k_ent,0.,1.05*k_ent) ;
			
			// RooAddPdf k_model("k_model","k_model",RooArgList(k_sig,k_bgn),RooArgList(k_N_s,k_N_b)) ;
			// 
			// RooAbsReal* k_nll = k_model.createNLL(data_ks,Extended(true)) ;
			// RooMinuit k_minu(*k_nll);
				// 
			// k_minu.setPrintLevel(-1);
			// k_minu.setNoWarn();
			// 
			// k_minu.hesse();
			// k_minu.simplex();
			...
			// cout << setw(10) << k_mean.getVal();
			// cout << setw(10) << k_sigma1.getVal();
			// cout << setw(10) << k_sigma2.getVal();
			// cout << setw(10) << k_frac_s.getVal();
			// cout << setw(10) << k_n.getVal();
			// cout << setw(10) << k_N_s.getVal();
			// cout << setw(10) << k_a.getVal() << endl;
			
			
			/***************/
			/*  Rho fit    */
			/***************/
			RooRealVar N_ks("N_ks","N_ks",k_N_s.getVal()) ;
			...
			RooRealVar sigma2("sigma2","sigma2",99.,0.,100.,"MeV");
...
// 			RooRealVar N_a_s("N_a_s","N_a_s",0.95*ent,0.,1.05*ent) ;
			...
			RooAddPdf model_all("model_all","model_all",RooArgList(sig,bgna),RooArgList(N_a_s,N_a_b)) ;
			RooAddPdf model_pi("model_pi","model_pi",RooArgList(sig,bgna),RooArgList(N_pi_s,N_pi_b)) ; // checked with bgnpi
			RooAddPdf model_k("model_k","model_k",RooArgList(sig,bgnk,bgauss),RooArgList(N_k_s,N_k_b, N_ks)) ; // checked with bgnk 
			RooAddPdf model_p("model_p","model_p",RooArgList(sig,bgnp),RooArgList(N_p_s,N_p_b)) ;
			RooAddPdf model_unk("model_unk","model_unk",RooArgList(sig,bgna),RooArgList(N_u_s,N_u_b)) ;
			
			RooDataHist data_all("data_all","data_all",x,Import(*h[6+cc][0][p][t])); //[p][t]
			RooDataHist data_pi("data_pi","data_pi",x,Import(*h[6+cc][1][p][t])); //[p][t]
			RooDataHist data_k("data_k","data_k",x,Import(*h[6+cc][2][p][t])); //[p][t]
			RooDataHist data_p("data_p","data_p",x,Import(*h[6+cc][3][p][t])); //[p][t]
			RooDataHist data_unk("data_unk","data_unk",x,Import(*h[6+cc][4][p][t])); //[p][t]
			...
			RooRealVar b2mean("b2mean","mean",910,850,1000,"MeV") ;
			RooRealVar b2sigma("b2sigma","sigma",34,15,50,"MeV");
			RooGaussian b2gauss("b2gauss","gauss",x2,b2mean,b2sigma) ;
			
			RooAddPdf k_model("k_model","k_model",RooArgList(k_sig,k_bgn,b2gauss),RooArgList(k_N_s,k_N_b,N2)) ;
...
// 					delta = 1 - (N_pi_s.getVal() + N_k_s.getVal() + N_p_s.getVal() + N_u_s.getVal()) / N_a_s.getVal();
				...
				}while( r[6+cc][p][t]->covQual() != 3 && i<= 0);
				
				cout << N_ks.getVal() << " " << N_k_s.getVal() << endl;
				}
			}else {
...
// 			simPdf.plotOn(frame3,Slice(sample,"k"),ProjWData(sample,combData),LineWidth(lw),VisualizeError(*r,1),FillColor(kCyan)) ;
			...
			simPdf.plotOn(frame3,Slice(sample,"k"),Components("gauss1"),ProjWData(sample,combData),LineStyle(kDashed),LineWidth(lw),LineColor(kRed)) ;
			simPdf.plotOn(frame3,Slice(sample,"k"),Components("gauss2"),ProjWData(sample,combData),LineStyle(kDashed),LineWidth(lw),LineColor(kGreen)) ;
			simPdf.plotOn(frame3,Slice(sample,"k"),Components("bgauss"),ProjWData(sample,combData),LineStyle(kDashed),LineWidth(lw),LineColor(kGray)) ;
			simPdf.plotOn(frame3,Slice(sample,"k"),Components("bgnk"),ProjWData(sample,combData),LineStyle(kDashed),LineWidth(lw),LineColor(kOrange)) ;
			combData.plotOn(frame3,Cut("sample==sample::k")) ;
...
// 			simPdf.plotOn(frame5,Slice(sample,"unk"),ProjWData(sample,combData),LineWidth(lw),VisualizeError(*r,1),FillColor(kCyan)) ;
			...
			RooPlot* frame6 = x2.frame(Title("K^{*}")) ;
			data_ks.plotOn(frame6) ;
			k_model.plotOn(frame6,LineWidth(lw)) ;
			k_model.plotOn(frame6,Components("k_gauss1"),LineStyle(kDashed),LineWidth(lw),LineColor(kRed)) ;
			k_model.plotOn(frame6,Components("k_gauss2"),LineStyle(kDashed),LineWidth(lw),LineColor(kGreen)) ;
			k_model.plotOn(frame6,Components("k_bgn"),LineStyle(kDashed),LineWidth(lw),LineColor(kOrange)) ;
			k_model.plotOn(frame6,Components("b2gauss"),LineStyle(kDashed),LineWidth(lw),LineColor(kOrange)) ;
			data_ks.plotOn(frame6) ;
